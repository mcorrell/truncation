...
const markSize = 10;
const lineWidth = 4;
const x = d3.scaleBand().domain([0, 1]).range([margin, vizWidth - margin]).paddingOuter(0.2).paddingInner(0.1);
const y = d3.scaleLinear().domain([0, 1]).range([vizHeight - margin, margin]);
const yAxis = d3.axisLeft(y).tickFormat(d3.format(".0%")).tickValues([0, 1]);
...
function makeStimuli(permute){
  ...
  for (let i = 0;i < trainingReplications;i++){
    for (let truncation of [truncationTypes[0], truncationTypes[truncationTypes.length - 1]]){
      for (let slope of [slopes[0], slopes[slopes.length - 1]]){
        stimulis = {};
  ...
  for (let vis of visTypes){
    for (let label of labelTypes){
      for (let frame of framingTypes){
        for (let truncation of truncationTypes){
...
var makeInput = function(parent, id, question){
  parent.append("div")
    .text(question)
...
var brokenBar = function(svg, data){
  ...
  let denom = 4;
  let breakPoint = 1 / denom;
  while (breakPoint > t){
    denom++;
    breakPoint = 1 / denom;
  }

  const breakMargin = 10;
  let y1 = d3.scaleLinear().domain([0, breakPoint]).range([vizHeight - 20, vizHeight - margin + breakMargin]).clamp(true);
  let y2 = d3.scaleLinear().domain([t, 1]).range([vizHeight - margin, margin]);
  svg.datum(data);
  x.domain(dl.range(0, data.length, 1));
...
var brokenGradBar = function(svg, data){
  ...
  let y1Axis = d3.axisLeft(y1).tickFormat(d3.format(".0%")).tickValues([0]);
  let y2Axis = d3.axisLeft(y2).tickFormat(d3.format(".0%")).tickValues([t, 1]);
